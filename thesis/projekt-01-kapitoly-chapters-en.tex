...
\chapter{Preface}
The following notation conventions apply throughout the thesis if not explicitly stated otherwise.  In the context pertaining to physics, symbols in \textbf{bold} represent vectors and symbols in \emph{italics} represent scalars, unless stated differently. The del operator denoted by the nabla symbol $\pmb{\nabla}$ is used with the dot product and cross product to denote divergence: $\pmb{\nabla} \cdot \mathbf{v}$ and the curl: $\pmb{\nabla} \times \mathbf{v}$, this is explaind more in \cref{sec:operators}.
...
The nabla operator can be used to define several important operations in vector calculus, such as the \emph{gradient}, \emph{divergence}, and \emph{curl}.
...
%\nabla \times \mathbf{B} =\mu_0\mathbf{J} + \mu_0 \varepsilon_0 \frac{\partial \mathbf{D}}{\partial t}
\end{equation}
...
%Where $\mathbf{B}$ is the magnetic field, $\mathbf{E}$ is the electric field, $\mathbf{J}$ the current density, $\mu_0$ the vacuum permeability, $\varepsilon_0 = $ the vacuum permittivity. It also holds that the speed of light $c=\left(\varepsilon_0\mu_0\right)^{-\frac{1}{2}}$.
...
Where $n_1$ is the refractive index of the first medium and $n_2$ is the refractive index of the second medium. This is shown in \cref{fig:snell} bellow.
...
\chapter{Evaluation of Results}\label{chap:results}
...
\begin{figure}[H]\label{fig:single-interface}
  \centering
...
\todo{It is also possible that the observed increase in computational time could be attributed to }
...
\section{Future Work}
\todo{implement more commands}
\todo{error recovery}
\todo{abstract more code into runtime}
